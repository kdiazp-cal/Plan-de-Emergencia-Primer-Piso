...
const alertBanner = document.getElementById('alertOverlay');
...
function applyLocalAlertOverride() {
  try {
    const mode = localStorage.getItem('localAlertOverrideMode');
    if (mode === 'off') {
...
if (adminForm) {
  adminForm.addEventListener('submit', (e) => {
    ...
    localStorage.setItem('localAlertOverrideMode', state);
...
async function pollAlert() {
  ...
  try {
    const res = await fetch('./alerta.json?t=' + Date.now(), { cache: 'no-store', headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' } });
    if (!res.ok) return;
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') pollAlert();
});
